...
export default function PickListsPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [pl, items] = await Promise.all([
      fetch("/api/materials/pick-lists").then((r) => r.json()),
      fetch("/api/materials/items").then((r) => r.json()),
    ]);
    setPickLists(pl);
    setAllItems(items);
    setLoading(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const url = editingList ? `/api/materials/pick-lists/${editingList.id}` : "/api/materials/pick-lists";
  ...
  async function handleArchive() {
    ...
    await fetch(`/api/materials/pick-lists/${archiveTarget.id}`, { method: "DELETE" });
