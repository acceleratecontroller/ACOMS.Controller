    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-4">
        <input
          ...
          className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm w-full sm:w-80 dark:bg-gray-700 dark:text-gray-100"
        />
        <select
          value={stockFilter}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="space-y-3 md:hidden">
            {jobs.map((job) => (
              <div key={job.id} onClick={() => router.push(`/materials/jobs/${job.id}`)} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-3 overflow-hidden cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="min-w-0">
                    <span className="font-mono text-blue-600 font-medium text-sm break-all">
                      {job.projectId}
                    </span>
                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100 mt-0.5 truncate">{job.name}</div>
                  </div>
                  <div className="text-right shrink-0">
                    {(job._count.materials > 0 || job._count.movements > 0) ? (
                      ...
                      <span className="inline-block px-1.5 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500">
                        No stock
                      </span>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-sm mt-2">
                  <div className="min-w-0">
                    ...
                    <div className="text-gray-700 dark:text-gray-300 truncate">{job.client}</div>
                  </div>
                  <div className="min-w-0">
                    <span className="text-gray-400 dark:text-gray-500 text-xs">Depot</span>
                    <div className="text-gray-700 dark:text-gray-300 truncate">{job.depotName || job.location?.name || "—"}</div>
                  </div>
                  {job.stage && (
                    <div className="min-w-0">
                      <span className="text-gray-400 dark:text-gray-500 text-xs">Stage</span>
                      <div className="text-gray-700 dark:text-gray-300 truncate capitalize">{job.stage.replace(/_/g, " ")}</div>
                    </div>
                  )}
                  {job.contact && (
                    <div className="min-w-0">
                      <span className="text-gray-400 dark:text-gray-500 text-xs">Contact</span>
                      <div className="text-gray-700 dark:text-gray-300 truncate">{job.contact}</div>
                    </div>
                  )}
                </div>
                {!showArchived && (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-x-auto hidden md:block">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                <tr>
                  <th className="text-left px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Project ID</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Job Name</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Client</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Depot</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Stage</th>
                  <th className="text-right px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Movements</th>
                  {!showArchived && <th className="text-right px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Actions</th>}
                </tr>
              </thead>
              <tbody>
                {jobs.map((job) => (
                  <tr key={job.id} onClick={() => router.push(`/materials/jobs/${job.id}`)} className="border-b border-gray-100 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer">
                    <td className="px-4 py-3">
                      <span className="font-mono text-blue-600 font-medium">
                        {job.projectId}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{job.name}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{job.client}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{job.depotName || job.location?.name || "—"}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 capitalize">{job.stage ? job.stage.replace(/_/g, " ") : "—"}</td>
                    <td className="px-4 py-3 text-right text-gray-500 dark:text-gray-400">{job._count.movements}</td>
                    {!showArchived && (
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Link WIP Job" wide>
        <div className="space-y-4">
          ...
          {wipLoading ? (
            <p className="text-gray-500 dark:text-gray-400 text-sm py-4">Loading WIP jobs...</p>
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden max-h-96 overflow-y-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700 sticky top-0">
                  <tr>
                    <th className="text-left px-3 py-2 font-medium text-gray-700 dark:text-gray-300">ACOMS #</th>
                    <th className="text-left px-3 py-2 font-medium text-gray-700 dark:text-gray-300">Job Name</th>
                    <th className="text-left px-3 py-2 font-medium text-gray-700 dark:text-gray-300">Client</th>
                    <th className="text-left px-3 py-2 font-medium text-gray-700 dark:text-gray-300">Depot</th>
                    <th className="text-left px-3 py-2 font-medium text-gray-700 dark:text-gray-300">Stage</th>
                    <th className="text-right px-3 py-2 font-medium text-gray-700 dark:text-gray-300"></th>
                  </tr>
                </thead>
                <tbody>
                  {wipProjects.map((wp) => (
                    <tr key={wp.id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-blue-900/20">
                      <td className="px-3 py-2 font-mono text-blue-600 font-medium">{wp.acomsNumberFormatted}</td>
                      <td className="px-3 py-2 text-gray-700 dark:text-gray-300">{wp.name}</td>
                      <td className="px-3 py-2 text-gray-500 dark:text-gray-400">{wp.clientName || "—"}</td>
                      <td className="px-3 py-2 text-gray-500 dark:text-gray-400">{wp.depotName || "—"}</td>
                      <td className="px-3 py-2 text-gray-500 dark:text-gray-400 capitalize">{wp.stage.replace(/_/g, " ")}</td>
      ...
      <Modal isOpen={!!archiveJob} onClose={closeArchiveModal} title={`Archive Job — ${archiveJob?.projectId}`} wide>
        {archiveLoading ? (
          <p className="text-gray-500 dark:text-gray-400 text-sm py-4">Loading job details...</p>
        ) : archiveDetail && (
          <div className="space-y-4">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              <span className="font-medium text-gray-900 dark:text-gray-100">{archiveDetail.name}</span>
            ...
            {archiveMaterials.length > 0 ? (
              <>
                <div>
                  <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-2">Materials on this job</h3>
                  <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                        <tr>
                          <th className="text-left px-3 py-2 font-medium text-gray-700 dark:text-gray-300">Item</th>
                          <th className="text-right px-3 py-2 font-medium text-gray-700 dark:text-gray-300">From Stock</th>
                          <th className="text-right px-3 py-2 font-medium text-gray-700 dark:text-gray-300">Received</th>
                          <th className="text-right px-3 py-2 font-medium text-gray-700 dark:text-gray-300">Total</th>
                        </tr>
                      </thead>
                      <tbody>
                        {archiveMaterials.map((mat) => (
                          <tr key={mat.itemId} className="border-b border-gray-100 dark:border-gray-700">
                            <td className="px-3 py-2">
                              <span className="font-mono text-xs text-gray-500 dark:text-gray-400">{mat.code}</span>{" "}
                              <span className="text-gray-700 dark:text-gray-300">{mat.description}</span>
                            </td>
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-2">What should happen to these materials?</h3>
                  <div className="space-y-2">
                    <label
                      className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer ${disposition === "RETURN_TO_STOCK" ? "border-blue-500 bg-blue-50" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700"}`}
                    >
                      <input
                        type="radio"
                        name="disposition"
                      ...
                      />
                      <div>
                        <div className="text-sm font-medium text-gray-900 dark:text-gray-100">Add back to stock</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">Materials will be returned as unallocated stock at {archiveDetail.location?.name || "the current location"}</div>
                      </div>
                    </label>
                    <label
                      className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer ${disposition === "RETURN_TO_CLIENT" ? "border-blue-500 bg-blue-50" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700"}`}
                    >
                      <input
                        type="radio"
                        name="disposition"
                      ...
                      />
                      <div>
                        <div className="text-sm font-medium text-gray-900 dark:text-gray-100">Return to client</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">Materials will be tracked in the Returns queue for return to the client</div>
                      </div>
                    </label>
                  </div>
                </div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 py-2">No materials on this job. It will be archived directly.</p>
            )}
